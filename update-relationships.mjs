...
async function updateField(collection, fieldName, parentCollection) {
  try {
    const updateData = {
      meta: {
    ...
    if (response.ok) {
      console.log(`✅ ${collection}.${fieldName} → ${parentCollection}`);
      return true;
    } else {
    ...
    console.error(`Error updating ${collection}.${fieldName}:`, error.message);
    return false;
  }
}

async function updateAllRelationships() {
  console.log('Updating relationship configurations in Directus...\n');

  for (const rel of relationshipsToUpdate) {
    await updateField(rel.collection, rel.fieldName, rel.parentCollection);
