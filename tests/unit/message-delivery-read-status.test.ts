...
describe('Message Delivery & Read Status - Backend Integration Tests', () => {
    ...
    try {
      if (driverSocket) {
        if (driverSocket.connected) {
          await driverSocket.disconnect()
        } else {
          console.log('⚠️ Driver socket not connected, skipping disconnect')
        }
      }
    } catch (error) {
      console.warn('⚠️ Error disconnecting driver socket:', error.message)
    }

    try {
      if (shipperSocket) {
        if (shipperSocket.connected) {
          await shipperSocket.disconnect()
        } else {
          console.log('⚠️ Shipper socket not connected, skipping disconnect')
        }
      }
    } catch (error) {
      console.warn('⚠️ Error disconnecting shipper socket:', error.message)
    }
  ...
  async function loginUser(email, password) {
    try {
      ...
      console.log(`📋 Login response for ${email}:`, response.data)
  ...
  async function connectSocket(userId, token, role) {
    return new Promise((resolve, reject) => {
      ...
      socket.on('connect', () => {
        ...
        console.log(`✅ Socket connected for ${role}: ${socket.id}`)
        
        socket.emit('register_user', { user_id: userId, role })
        
        socket.on('user_registered', () => {
          console.log(`✅ User registered on socket for ${role}`)
        })
  ...
  async function getMessageReadStatus(messageId, token) {
    try {
      ...
      console.error('❌ Failed to get message read status:', error)
      throw error
    }
  }
  ...
  it('Should create sockets for driver and shipper', async () => {
    try {
      ...
      console.warn('⚠️ Socket connection not available, using API-only mode:', error.message)
      console.log('✅ API-based message delivery will still work')
    }
  }, 60000)
  ...
  it('Should send a message from driver and show SENT status', async () => {
    ...
    console.log('✅ Message sent with initial status')
  }, 30000)
  ...
  it('Should update message to DELIVERED status when recipient connects', async () => {
    ...
    console.log('✅ Message delivery status confirmed')
  }, 30000)
  ...
  it('Should maintain correct status progression: SENT -> DELIVERED -> READ', async () => {
    ...
    expect(message.id).toBeGreaterThan(0)
    console.log(`📤 Step 1: Message sent with ID ${message.id}`)

    await new Promise(resolve => setTimeout(resolve, 500))

    const deliveredStatus = await getMessageReadStatus(message.id, authTokens.driver)
    console.log(`📋 Step 2: Initial delivery status: ${JSON.stringify(deliveredStatus)}`)
    ...
    expect(readRecord.status).toBe('read')
    console.log(`✅ Step 3: Message marked as read`)
    ...
    console.log('✅ Status progression verified: SENT → DELIVERED → READ')
  }, 30000)
})
